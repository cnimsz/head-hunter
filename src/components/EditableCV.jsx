const input =
  'w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm';
  ...
  return (
    <div className="flex-1 overflow-auto space-y-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded p-3">
      ...
      <div>
        ...
        <input
          className={input}
          placeholder="e.g. Chief Operating Officer · Chief Strategy Officer"
          value={data.title || ''}
        ...
        {(data.experience || []).map((role, ri) => (
          <div key={ri} className="border border-slate-200 dark:border-slate-700 rounded p-2 mb-2">
            ...
            <input
              ...
              placeholder="Title (e.g. COO — AI for Enterprise)"
              value={role.title || ''}
              onChange={(e) => set(`experience[${ri}].title`, e.target.value)}
            />
            <div className="flex gap-1 mb-1">
              <input
                ...
                value={role.location || ''}
                onChange={(e) => set(`experience[${ri}].location`, e.target.value)}
              />
              <input
                className={`${input} w-32`}
                placeholder="Start (Jan 2024)"
                value={role.startDate || ''}
                onChange={(e) => set(`experience[${ri}].startDate`, e.target.value)}
              />
              <input
                className={`${input} w-32`}
                placeholder="End (Present)"
                value={role.endDate || ''}
                onChange={(e) => set(`experience[${ri}].endDate`, e.target.value)}
              />
            </div>
            <input
              className={`${input} mb-1 text-xs text-slate-500`}
              placeholder="titleLine (legacy fallback)"
              value={role.titleLine || ''}
              onChange={(e) => set(`experience[${ri}].titleLine`, e.target.value)}
            />
        ...
        {(data.startupAchievements || []).map((a, i) => (
          <div key={i} className="border border-slate-200 dark:border-slate-700 rounded p-2 mb-2">
            ...
            <textarea
              ...
              placeholder="Body"
              value={a.body || ''}
              onChange={(e) => set(`startupAchievements[${i}].body`, e.target.value)}
            />
          </div>
        ))}
      </div>
