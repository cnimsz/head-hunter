...
export default function EditableCV({ data, onChange }) {
  ...
  function addRole() {
    const d = structuredClone(data);
    d.experience.push({ company: '', titleLine: '', bullets: [''] });
    onChange(d);
  }
      ...
      <div>
        ...
        {(data.experience || []).map((role, ri) => (
          <div key={ri} className="border border-slate-200 dark:border-slate-700 rounded p-2 mb-2">
            ...
            <input className={`${input} mb-1`} placeholder="Title | Location | Dates" value={role.titleLine} onChange={(e) => set(`experience[${ri}].titleLine`, e.target.value)} />
